...
class MatricularMateria(Frame):
    def __init__(self, ventana):
        ...
        self.config(highlightbackground="#089970", highlightthickness=0)

        titulo = Label(self, text="Matricular Materia", foreground="#089970", font=("Helvetica", 14, "bold"))
        ...
        2. Búsqueda por lista: Selecciona el estudiante de una lista disponible """
        descripcion = Label(self, text=texto, font=("Arial", 11))
        descripcion.pack(anchor="n", pady=10)
        opciones = Frame(self)
        opciones.pack()
        manual = Button(opciones, text="Seleccionar al estudiante\n de forma manual", foreground="white", background="#089970", font=("Helvetica", 12), command=lambda : self.seleccionar_estudiante(caja, label_vacio, False))
        lista = Button(opciones, text="Seleccionar al estudiante\n mediante una lista", foreground="white", background="#089970", font=("Helvetica", 12), command=lambda : self.seleccionar_estudiante(caja, label_vacio, True))
    ...
    def seleccionar_estudiante(self, frame, label, busqueda_lista=False):
        ...
        if busqueda_lista:
            label.comfig(text="Entro a búsqueda por lista",)
            nombres_texto = ""
            ...
            MatricularMateria.limpiar_frame(frame)
            label.config(text="Entro a búsqueda manual",)
...
class MatricularMateria2(Frame):
    def __init__(self, ventana, estudiante):
        ...
        self.pack(expand=True)
        self.config(highlightbackground="#089970", highlightthickness=0)
        titulo = Label(self, text="Matricular Materia 2", foreground="#089970", font=("Helvetica", 14, "bold"))
        titulo.pack(side="top", anchor="c")
        ...
        opciones = Frame(self)
        opciones.pack()
        manual = Button(opciones, text="Seleccionar la materia\n de forma manual", foreground="white", background="#089970", font=("Helvetica", 12), command= lambda : self.seleccionar_materia(caja, label_vacio, False))
        lista = Button(opciones, text="Seleccionar la materia\n mediante una lista", foreground="white", background="#089970", font=("Helvetica", 12), command= lambda : self.seleccionar_materia(caja, label_vacio, True))
        manual.pack(side=LEFT, padx=(0,10))
        lista.pack(side=LEFT, padx= (10,10))
        busqueda = Frame(self)
        busqueda.pack()
    ...
    def seleccionar_materia(self, ventana, label, busqueda_lista = False ):
        ...
        if busqueda_lista:
            ...
            label.config(text="Entro a búsqueda por lista")
            materias_texto = ""
...
class MatricularMateria3(Frame):
    def __init__(self, ventana, estudiante, materia):
        ...
        titulo = Label(self, text="Matricular Materia 3", foreground="#089970", font=("Helvetica", 14, "bold"))
        titulo.pack(side="top", anchor="c")
        ...
        if opcion == "":
            ...
            try:
                ...
                if opcion <= len(self.grupos_disponibles) and opcion >= 1:
                    ...
                    self.horario_estudiante.ocuparHorario(self.horario_estudiante)
                    imprimir = "Materia " + self.materia.getNombre() + " - grupo #" + str(grupo_seleccionado.getNumero())
                    imprimir += ". Ha sido matriculado el estudiante: " + self.estudiante.getNombre()
...
class MatricularMateria4(Frame):
    def __init__(self, ventana, estudiante):
        ...
        titulo = Label(self, text="Mostrar horario estudiante", foreground="#089970", font=("Helvetica", 14, "bold"))
        titulo.pack(side="top", anchor="c")
        ...
        boton = Button(caja, text="Regresar", foreground="white", background="#089970", font=("Helvetica", 12), command=self.regresar)
        boton.pack(pady=10)
