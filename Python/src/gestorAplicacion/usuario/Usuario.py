...
class Usuario(ABC):
    ...
    def mostrarUsuarios(cls) -> str:
        ...
        for usuario in cls.getUsuariosTotales():
            retorno += (str(i) + ". " + usuario._nombre + ". id:" + str(usuario._id) + "\n")
            i += 1
        return retorno
    ...
    def desmatricularDelSistema(self, usuario) -> None:
    ...
    def eliminar_materia(self, materia) -> None:
        Materia.getMateriasTotales().remove(materia)
    

    def agregar_materia(self, nombre, codigo, descripcion, creditos, facultad, prerrequisitos) -> None:
